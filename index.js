...
app.get("/filter", (req, res) => {
  ...
  const found_joke = jokes.filter((joke) => joke.jokeType === type);
  res.json(found_joke);
});
...
app.put("/jokes/:id", (req, res) => {
  ...
  const found_joke = jokes.findIndex((joke) => joke.id === id);

  jokes[found_joke] = replacement_joke;
  res.json(replacement_joke);
});

app.patch("/jokes/:id", (req, res) => {
  const id = parseInt(req.params.id);
  const match = jokes.find((joke) => joke.id === id);
  const newJoke = {
    id: id,
    jokeText: req.body.text || match.jokeText,
    jokeType: req.body.type || match.jokeType,
  };
...
app.delete("/jokes/:id", (req, res) => {
  ...
  if (index > -1) {
    ...
    res.status(400).json({ error: `Joke with id ${id} no found` });
  }
});
